...
class VkApi {
   ...
   */
  authorize ({ client = 'android', login = this.options.userLogin, password = this.options.userPassword, scope = constants.MAX_SCOPE } = {}, params = {}) {
    ...
    return new Promise(resolve => setTimeout(() => resolve(), this._getRequestDelayTime()))
      .then(() => fetch('https://oauth.vk.com/token', {
        qs: Object.assign({
          client_id:     clientId,
      ...
      .catch(error => {
        // Captcha needed.
        if (error.name === 'VkAuthError' && error.message === 'need_captcha' && this.captchaRecognizer) {
